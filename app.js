...
let castTime = 0; // Tambahan untuk track waktu floating
...
castBtn.addEventListener('click', ()=> {
  ...
  setTimeout(()=>{
    ...
    spawnRipple();
    flowState = 'floating';
    castTime = Date.now(); // Set waktu mulai floating
...
reelBtn.addEventListener('click', ()=> {
  ...
  setTimeout(()=>{
    ...
    if (timeFloating < 2) { // Minimal 2 detik floating untuk chance ikan
      ...
      catchChance = Math.min(0.6, catchChance * (timeFloating / 5)); // Scale chance berdasarkan waktu floating (max di 5 detik+)
...
// Touch & mouse on reelBtn: press and hold to influence when pulling
reelBtn.addEventListener('pointerdown', (e)=> {
  if(flowState === 'pulling') setHold(true);
});
reelBtn.addEventListener('pointerup', (e)=> {
  if(flowState === 'pulling') setHold(false);
});
